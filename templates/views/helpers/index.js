...
module.exports = function () {
    ...
    var f = options.hash.format || ' DD MM YYYY';
	...
	// output. 'Filed Undder <a href="engineeringjobs/tech">tech</a>, <a href="engineeringjobs/js">js</a>'
	...
	_helpers.postUrl = function (postSlug, options) {
		return ('/engineeringjobs/post/' + postSlug);
	};
	...
	_helpers.pageUrl = function (pageNumber, options) {
		return '/engineeringjobs?page=' + pageNumber;
	};

	//create the category url for a engineering-category page
	_helpers.categoryUrl = function (categorySlug, options) {
		return ('/engineeringjobs/' + categorySlug);
	};

  // Direct url link to a specific bank bankjob
  _helpers.bankjobUrl = function (bankjobSlug, options) {
   return ('/bankjobs/' + bankjobSlug);
  };

  // Direct url link to a specific upsc upscjob
  _helpers.upscjobUrl = function (upscjobSlug, options) {
   return ('/upscjobs/' + upscjobSlug);
  };
  // Direct url link to a specific gov govjob
  _helpers.govtjobUrl = function (govtjobSlug, options) {
   return ('/govtjobs/' + govtjobSlug);
  };

  // Direct url link to a specific railwayjob
  _helpers.railwayjobUrl = function (railwayjobSlug, options) {
   return ('/railwayjobs/' + railwayjobSlug);
  };

  // Direct url link to a specific itjob
  _helpers.itjobUrl = function (itjobSlug, options) {
   return ('/itjobs/' + itjobSlug);
  };
	// ### Pagination Helpers
